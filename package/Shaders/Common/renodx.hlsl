...
{
    return (divisor == 0.f)
               ? asfloat(0x7F7FFFFF) * Sign(dividend)
               : (dividend / divisor);
}
...
float3 BT709(float3 bt709, Config current_config)
{
    ...
    [branch]
    if (current_config.dechroma != 0.f || current_config.saturation != 1.f || current_config.hue_correction_strength != 0.f || current_config.blowout != 0.f)
    {
