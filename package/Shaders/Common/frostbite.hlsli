...
namespace tonemap
{
	namespace frostbite
	{
		...
		/// Tonemaps a BT.709 color through an ICtCp transformation.
		/// It uses a combination of hue-preserving and non-hue-preserving transformations to maintain
		/// a natural appearance while compressing the dynamic range.
		///
		/// @param col Input color in BT.709 color space.
		/// @param max_value The maximum value to which the dynamic range is compressed.
		/// @param shoulder_start The point at which luminance compression begins.
		/// @return The tone-mapped color in BT.709 color space.
		float3 BT709(float3 col, float max_value = 1.f, float shoulder_start = 0.25, float hue_preserve_strength = 0.6)
		{
			float3 ictcp = tonemap::frostbite::color::RGBToICtCp(col);
			...
			col = tonemap::frostbite::color::ICtCpToRGB(ictcp * float3(1, saturationAmount.xx));

			// Only compress luminance starting at a certain point. Dimmer inputs are passed through without modification.
			float linearSegmentEnd = shoulder_start;
			...
			col = lerp(perChannelCompressed, compressedHuePreserving, hue_preserve_strength);

			float3 ictcpMapped = tonemap::frostbite::color::RGBToICtCp(col);
			...
			col = tonemap::frostbite::color::ICtCpToRGB(ictcpMapped);
